...
function partitionDLL(DLL, pivot) {
    var headLower, tailLower = null;
    var headHigher, tailHigher = null;
    var headEqual, tailEqual = null;

    while (DLL.head != null) {
        if (DLL.head.data == pivot) {
            if (headEqual == null)
                headEqual = tailEqual = DLL.head;
            else {
                tailEqual.next = DLL.head;
                tailEqual = tailEqual.next; //
            } //yeah this is a weird algorithm
        } else if (DLL.head.data < x) {
            if (headLower == null)
                headLower = tailLower = DLL.head;
            else {
                tailLower.next = DLL.head;
                tailLower = DLL.head;
            }
        } else {
            if (headHigher == null)
                tailHigher = headHigher = DLL.head;
            else {
                tailHigher.next = DLL.head;
                tailHigher = DLL.head;
            }
        }
    ...
    if (headLower == null) {
        ...
        tailEqual.next = headHigher;
        return headEqual;
    }

    if (headEqual == null) {
        tailLower.next = headHigher;
        return headLower;
    }

    tailLower.next = headEqual;
    tailEqual.next = headHigher;
    return headLower;
}

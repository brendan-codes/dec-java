...
function partitionDLL(DLL, pivot) {
    ...
    while (runner != null) { // saves values to left and right arrays
    ...
    for (var i = 0; i < right.length; i++) { // Add greater than values to the new DLL
        newDLL.addHead(new Node(right[i]));
    }

    newDLL.addHead(new Node(pivot)); // Add pivot node

    for (var i = 0; i < left.length; i++) { // Add lesser than values to the new DLL
        newDLL.addHead(new Node(left[i]));
    }

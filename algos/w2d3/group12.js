...
function isPalindrome(queue) {
    let stack = new slStack(); // create a stack so we're comparing front to end as we remove elements
    let palindrome = true;
    for (var i = 0; i < queue.length; i++) { // add all nodes to the stack
        let node = queue.dequeue();
        stack.push(new Node(node.data));
        queue.enqueue(node);
    }
    for (var i = 0; i < queue.length; i++) { // check if it's a palindrome
        let nodeQueue = queue.dequeue();
        let nodeStack = stack.pop();
        if (nodeQueue.data !== nodeStack.data) { // check if the first node in each matches
            palindrome = false;
        }
        queue.enqueue(nodeQueue);
    }
    return palindrome;
}
